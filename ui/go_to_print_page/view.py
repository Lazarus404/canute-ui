...
def render_help_menu(width, height, page):
    data = (
        'Go to a print page number by keying it',
        'in with the side number buttons and',
        'pressing forward. Pages are numbered',
        'based on the #i line page height of the',
        'Canute. You can delete entered numbers',
        'by pressing or holding the back button.',
        'As always you can go back to your',
        'current page by pressing the menu',
        'button.',
    )
...
def render(width, height, state):
    ...
    selection = state['go_to_print_page']['selection']
    if selection == '':
        selection = -1
    else:
